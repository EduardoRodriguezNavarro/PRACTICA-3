...
int main(void)
{
    ...
    uint16_t valor = 0;
    char c='5';
    int Largo=200;//VARIABLE DONDE ALMACENARÉ EL LARGO DE MI ARREGLO
   ...
   // printString(&arr[0]);
    while(1)
    {
         ...
         switch(c)
         {
           ...
           //--------------LLAMADA A FUNCIÓN
           case 'x':
                 GPIOF->DATA = (1<<1) | (0<<2) | (0<<3);//PRENDEMOS TODOS LOS LED'S
                 Largo=readString(',',&Nombre[0]);//FUNCION QUE CALCULA LARGO DEL ARREGLO
                 NomIntvert(&Nombre[0],Largo);//FUNCION QUE INVIERTE Y AÑADE LOS NUMEROS
